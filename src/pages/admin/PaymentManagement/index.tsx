  ...
  const updateMutation = useMutation({
    mutationFn: async (payload: any) => {
      try {
        ...
        throw error; // Rethrow error to be caught by onError
      }
    },
    onSuccess: (res: any) => {
      if (res) {
        refetch(); // Refetch the data
        toast.success("Payment model updated successfully"); // Show success message
      } else {
        toast.error("Something went wrong");
      }
    },
    onError: (error: any) => {
      toast.error(error?.message || "An error occurred"); // Show error message
    },
  });
